...
export class CaslAbilityFactory {
   ...
   *
   * Config-gated rules (media, prizes) are only added when the event has the
   * corresponding module enabled, so toggling EventConfig automatically changes
   * what organizers are allowed to do after the cache expires.
   */
      ...
      switch (role) {
        ...
        case EventRole.ORGANIZER:
          if (isEnded) {
            ...
            if (
              config?.mediaAccess &&
              config.mediaAccess !== MediaAccess.DISABLED
            ) {
              rules.push({
                action: 'manage',
                subject: 'MediaItem',
